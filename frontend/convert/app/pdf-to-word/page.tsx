...
export default function PdfToWord() {
  ...
  const accentColor = "var(--accent-orange)";
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `pdf-to-word-${Date.now()}.docx`;
    ...
    <div className="page-container page-container-narrow">
      ...
      <div className="mb-8 animate-fadeInUp">
        <h1 className="text-3xl sm:text-4xl font-bold mb-3">
          <span style={{ color: accentColor }}>PDF</span>
          <span className="text-[var(--text-primary)]"> para Word</span>
        </h1>
        <p className="text-[var(--text-secondary)] text-lg">
          Converta arquivos PDF em documentos Word editaveis (.docx)
        </p>
      </div>
      ...
      {!file && !loading && (
        <div className="mb-6 animate-fadeInUp" style={{ animationDelay: "100ms" }}>
          <DropZone
            ...
            maxFiles={1}
            maxSize={50}
            label="Clique ou arraste seu arquivo PDF aqui"
      ...
      {/* Info Box */}
      <div className="mb-6 animate-fadeInUp" style={{ animationDelay: "250ms" }}>
        <div className="p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border)]">
          <h4 className="font-medium text-[var(--text-primary)] mb-2 flex items-center gap-2">
            <svg className="w-5 h-5" style={{ color: accentColor }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Conversao de Alta Qualidade
          </h4>
          <p className="text-sm text-[var(--text-secondary)] mb-3">
            Utilizamos a tecnologia <strong className="text-[var(--text-primary)]">pdf2docx</strong> que preserva:
          </p>
          <ul className="text-sm text-[var(--text-secondary)] space-y-1 ml-4">
            <li className="flex items-center gap-2">
              <span style={{ color: accentColor }}>+</span> Formatacao original (negrito, italico, cores)
            </li>
            <li className="flex items-center gap-2">
              <span style={{ color: accentColor }}>+</span> Imagens e graficos
            </li>
            <li className="flex items-center gap-2">
              <span style={{ color: accentColor }}>+</span> Tabelas e layout
            </li>
            <li className="flex items-center gap-2">
              <span style={{ color: accentColor }}>+</span> Estrutura do documento
            </li>
          </ul>
          <p className="text-xs text-[var(--text-muted)] mt-3 italic">
            Nota: PDFs escaneados (apenas imagens) podem ter resultados limitados.
          </p>
        </div>
      </div>
